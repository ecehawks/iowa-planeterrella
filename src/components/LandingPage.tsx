...
export default class LandingPage extends React.Component<LandingPageProps, LandingPageState> {
		...
		if (email !== '' && password !== '') {
			...
			firebase.auth().signInWithEmailAndPassword(email, password).then(function () {
				...
				queue_ref.once('value')
					.then(function (snapshot: any) {
						sessionStorage.setItem('queue', JSON.stringify(snapshot.val()));
					});
	...
	startTimer(type: string, duration: number) {
		...
		this.interval = setInterval(function () {
			...
			if (timer.isControl) {
				timerString = minutes + ' min ' + seconds + ' sec left';
				sessionStorage.setItem('timerString',timerString);
			} else {
				timerString = 'Approximately ' + minutes + ' min ' + seconds + ' sec until your turn';
				sessionStorage.setItem('timerString',timerString);
			}
